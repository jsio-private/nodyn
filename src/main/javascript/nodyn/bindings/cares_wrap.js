...
function translateError(err) {
  if ( err instanceof ClassHelpers.getClass("org.vertx.java.core.dns.DnsException") ) {
    var c = err.code().code();
    if ( c == 3 ) {
...
cares.getaddrinfo = function(req,name,family) {
  ...
  if ( family === 4 ) {
    this._query = new (ClassHelpers.getClass("io.nodyn.dns.GetAddrInfo4Wrap"))(process._process, name);
  } else if (family === 6 ) {
    this._query = new (ClassHelpers.getClass("io.nodyn.dns.GetAddrInfo6Wrap"))(process._process, name);
  } else {
    this._query = new (ClassHelpers.getClass("io.nodyn.dns.GetAddrInfoWrap"))(process._process, name);
  }
...
cares.queryA = function(req,name) {
  ...
  this._query = new (ClassHelpers.getClass("io.nodyn.dns.QueryAWrap"))(process._process, name);
  this._query.on( "complete", function(result) {
    if ( result.error ) {
...
cares.queryAaaa = function(req,name) {
  ...
  this._query = new (ClassHelpers.getClass("io.nodyn.dns.QueryAaaaWrap"))(process._process, name);
  this._query.on( "complete", function(result) {
    if ( result.error ) {
      req.oncomplete( result.error.toString() );
    } else {
...
cares.queryMx = function(req,name) {
  ...
  this._query = new (ClassHelpers.getClass("io.nodyn.dns.QueryMxWrap"))(process._process, name);
  this._query.on( "complete", function(result) {
    if ( result.error ) {
      req.oncomplete( result.error.toString() );
    } else {
...
cares.queryTxt = function(req,name) {
  ...
  this._query = new (ClassHelpers.getClass("io.nodyn.dns.QueryTxtWrap"))(process._process, name);
  this._query.on( "complete", function(result) {
    if ( result.error ) {
      req.oncomplete( result.error.toString() );
    } else {
...
cares.querySrv = function(req,name) {
  ...
  this._query = new (ClassHelpers.getClass("io.nodyn.dns.QuerySrvWrap"))(process._process, name);
  this._query.on( "complete", function(result) {
    if ( result.error ) {
      req.oncomplete( result.error.toString() );
    } else {
...
cares.queryNs = function(req,name) {
  ...
  this._query = new (ClassHelpers.getClass("io.nodyn.dns.QueryNsWrap"))(process._process, name);
  this._query.on( "complete", function(result) {
    if ( result.error ) {
      req.oncomplete( result.error.toString() );
    } else {
...
cares.queryCname = function(req,name) {
  ...
  this._query = new (ClassHelpers.getClass("io.nodyn.dns.QueryCnameWrap"))(process._process, name);
  this._query.on( "complete", function(result) {
    if ( result.error ) {
      req.oncomplete( result.error.toString() );
    } else {
...
cares.getHostByAddr = function(req,name) {
  ...
  this._query = new (ClassHelpers.getClass("io.nodyn.dns.GetHostByAddrWrap"))(process._process, name);
  this._query.on( "complete", function(result) {
    if ( result.error ) {
      req.oncomplete( result.error.toString() );
    } else {
